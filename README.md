...
> **Status:** UI scaffold with two genuinely live pieces — token prices
> and the USD→NGN rate are real, pulled live (see "What's actually real"
> below). Everything involving custody, identity verification, and moving
> real money is still mocked and marked `// TODO(integration)` — see
> `docs/TODO_INTEGRATIONS.md`. Wiring those up requires signed agreements
> with licensed providers (see the note at the bottom of this file).

## What's actually real vs. still mocked

| Area | Status |
|---|---|
| Token prices (`rateService.js`) | **Live** — CoinGecko public API, no key needed |
| USD→NGN rate (`rateService.js`) | **Live** — open.er-api.com, ₦20/$ margin applied on top per spec |
| Withdrawal fee math (`feeService.js`) | **Real business rule** ($0.10 revenue margin) on top of an **estimated** network fee — real network fee needs a live RPC/wallet-infra quote |
| Portfolio balances | Mock — no real custody yet, see "Key management" below |
| BVN/ID/selfie verification | Mock — needs a licensed KYC vendor account (Smile ID/VerifyMe/Prembly) |
| Auth (sign up/in) | **Live** — real Firebase Auth (email/password + email verification link) |
| User profiles / KYC status storage | **Live** — real Firestore, see `docs/firestore.rules` |
| Revenue/Liquidity wallet config | Config screen stores an address on-device; doesn't move real funds until a custody provider is wired in |
| Airtime & Data | Full mock purchase flow; needs a VTU aggregator (VTpass/ClubKonnect) for real delivery
...
2. Register a **Web app** (not Android/iOS) — even though this runs on
   mobile, the Firebase JS SDK used here expects web-style config
3. Copy the six config values into `.env` with an `EXPO_PUBLIC_FIREBASE_`
   prefix — see `.env.example`
4. Enable **Authentication → Sign-in method → Email/Password**
5. Create a **Firestore Database** (any region)
6. Replace the default "test mode" rules with `docs/firestore.rules` —
   test mode allows anyone to read/write everything, which is fine for a
   few days of development but not further than that
7. Restart the Expo server after editing `.env` — env vars only load on
   startup, not live
